...
import {format,addDays, subDays, differenceInDays,parse, isValid, getYear, getMonth } from 'date-fns';
import { diff } from 'util';

test ('ví dụ date picker', async ({page})=> {
    // // date là 1 đối tượng object trong JS/TS {}
    // //HN +7 ->UTC -7
    // const now = new Date ()
    ...
    // const dateB = new Date ('2025-11-20')
...
// => tìm tất cả các div có chứa class là ant-card
// là trong thẻ div đấy phải có chứa thẻ div con có chứa class ant-head-title và có chứa text () Demo 2 : Dropdown Navigation + Today highlight + Disable future dates
// quan trong nhất là dấu "."
// [//div[contains (@class, 'ant-card-head-title') and normalize - space ()= 'Demo 2: Dropdown Navigation + Today highlight + Disable future dates']]

const monthNames = [ 'January', 'February', 'March','April','May','June','July','August','September','October','November','December'];

async function openAntSelectedByCardTitle (page: Page, cardTitle: string,  index: number){
    const card = page.locator (
        `xpath=//[contains(@class, 'ant-card')] [.//div [contains (@class, 'ant-card-head-title') and normalize-space ()= '${cardTitle}']]`
    );
...
async function pickAntOptionByText (page: Page, text: string){
    const option = page.locator (
        `//div[contains(@class, 'ant-select-dropdown') and not (contains@style, 'display:none'))]//div[@class, 'ant-select-item-option-content') and normalize-space ()= '${text}'].first ()`
...
async function selectDateDemo2 (page: Page, ymd:string){
    ...
    const demo2Title = 'Demo 2: Dropdown Navigation + Today hightlight + Disable future dates';
